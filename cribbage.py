...
class CustomPlayer(shared.Player):
    def __init__(self, name="") -> None:
        super().__init__(name)
        self.unplayed_cards = []
        self.played_cards = []

class CustomRound(shared.Round):
    def __init__(self, player_order, num=0) -> None:
        super().__init__(player_order, num)
        self.crib = []
        self.starter = shared.Card("", 0, "", "") # "empty" Card class instead of None
...
class CustomState(shared.State):
    def __init__(self, ace_value=1, hand_size=5, max_players=3, random_names_flag=True, debug=False) -> None:
        super().__init__(ace_value, hand_size, max_players, random_names_flag)
        
        self.mode = "start"
        self.round = CustomRound(self.player_order)
    ...
    def custom_add_players(self, num_players: int) -> None:
        # name is stored as integer (order added)
        for i in range(num_players):
            self.players[i] = CustomPlayer()

    def new_round_custom(self):
        self.round = CustomRound(self.player_order, self.round.num+1)
        self.new_round(custom_round=True)
...
# main loop
def hotseat(random_names_flag=True, debug=False):
    state = CustomState(random_names_flag, debug=debug)
...
hotseat(random_names_flag=True, debug=False)