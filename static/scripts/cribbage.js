...
function animateToCrib(player, cardStrs, numToDiscard, cribSize) {
  ...
  for (let i = 0; i < numToDiscard; i++) {
    ...
    console.log(
      `card after processing: id - ${card.id}; class - ${card.className}`
    );
...
var handHandlerCribbage = function handOnClickCribbage(event) {
  ...
  if (mode === "discard") {
    ...
    if (
      ...
      addToLog(
        "You already selected enough cards and cannot select any more.",
        "system"
      );
      return;
    }
    ...
    if (
      ...
      toggleHandSelectability(
        (toggleOn = false),
        (excludedClasses = ["staged-for-discard"]),
        (excludedIds = [])
      );
    } else {
    ...
    socket.emit("move", {
      ...
      room: currentRoom,
      username: username,
      cards: [`${this.dataset.rank}${this.dataset.suit}`],
    });
  }
};
...
function updateCribbage(response) {
  ...
  for (actionObject of response.action_log) {
    ...
    console.log(
      `Action = ${actionObject.action}, Player = ${actionObject.player}, Cards = ${actionObject.cards}, num_to_discard = ${actionObject.num_to_discard}`
    );
  ...
  if (mode === "play" || response.action_log.length === 0) {
    ...
    if (response.starter) {
      ...
      document
        ...
        .replaceChildren(createCardObject(response.starter));
    }
  }
