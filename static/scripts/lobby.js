...
function createUsernameModal() {
  ...
  submitButton.className =
    "username-modal-button btn btn-primary form-control w-auto";
...
function createPasswordModal() {
  ...
  const submitButton = document.createElement("button");
  submitButton.className =
    "password-modal-button btn btn-primary form-control w-auto";
  ...
  cancelButton.className =
    "btn btn-secondary modal-cancel-button room-password";
  cancelButton.innerText = "Cancel";
  cancelButton.onclick = () => {
    closeModal(passwordModal);
  };
...
function setPasswordOnclick(roomToJoin) {
  ...
  if (
    document.querySelector("#room-tr-" + roomToJoin).dataset.roomUsername
      .length > 0
  ) {
    usernameInput.readOnly = true;
    usernameInput.value = document.querySelector(
      "#room-tr-" + roomToJoin
    ).dataset.roomUsername;
  }
...
function addRooms(newRooms) {
  ...
  for (const room of newRooms) {
    ...
    row.onclick = () => {
      ...
      if (room.pw_flag > 0) {
        reqUsername = document.querySelector(
          "#combined-username-modal-input"
        ).value;
      } else {
...
function updateLobby(response) {
  ...
  if (response.action === "setup_room") {
    ...
    ).innerText = `Lobby - ${GAME_DISPLAY_NAMES[chosenGame]}`;
    ...
    newRoomButton.className =
      "lobby-header-button btn btn-secondary form-control w-auto";
  ...
  // Update number of players in a room when a player leaves or joins
    ...
    if (response.col === "players") {
      document.querySelector("#room-td-players-" + response.row).innerText =
        response.new_value;
    }
    ...
    if (response.col === "in_progress") {
      // Update row dataset
      document.querySelector("#room-tr-" + response.row).dataset.inProgress =
        response.new_value;

      document.querySelector("#room-td-in_progress-" + response.row).innerText =
        response.new_value;
    }
  }
}
...
function createNewRoomModal() {
  ...
  const roomHelpText =
    "Room name must be 4-18 characters long and only contain letters, numbers, or underscores.";
  ...
  submitButton.onclick = () => {
    ...
    if (
      roomInput.value.length === 0 ||
      form.elements["game"].value === undefined
    ) {
    ...
    const promise = socket.emitWithAck("create_room", {
      new_room_name: roomInput.value,
      game: form.elements["game"].value,
...
async function preJoin(room, roompw, reqUsername) {
  ...
  try {
    ...
    // Using emitWithAck for callback; Returns promise.
    const response = await socket.emitWithAck("prejoin", {
      room: room,
      password: roompw,
...
socket.on("update_gameroom", (data) => {
  // For debug:
  console.log(
    `Client received 'update_gameroom' event: ${JSON.stringify(data)}.`
  );
