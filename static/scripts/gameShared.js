...
const ANIMATION_TIMING = { duration: 400, iterations: 1 };
...
function updateRuleModal(game) {
  ...
  document.querySelector(
    "#rules-modal-title"
  ).innerText = `Rules of ${GAME_DISPLAY_NAMES[game]}`;
...
function updateGameRoom(response) {
  ...
  if (response.action === "setup_room") {
    ...
    // Update page title
    document.querySelector("title").innerText = `The Space: ${
      GAME_DISPLAY_NAMES[response.game]
    }`;
    ...
    document.querySelector("#sub-header-center-h2").innerText = `${
      response.room
    } - ${GAME_DISPLAY_NAMES[response.game]}`;
    ...
    document
      .querySelector("#sub-header-right")
      .appendChild(createRulesButton());
    ...
    switch (response.game) {
      case "thirty_one":
        ...
        document
          ...
          .appendChild(createBoardThirtyOne());
        // Add move buttons to center of grid
        document
          .querySelector("#game-grid-5")
          .appendChild(createMoveButtonsThirtyOne());
        break;
    ...
    const chatLogPanel = gameRoomContainer.removeChild(
      document.querySelector("#chat-log-panel")
    );
  ...
  for (let j = 0; j < priorityOrder.length; j++) {
    ...
    if (gridsToFill.includes(priorityOrder[j])) {
      ...
      playerContainer = createPlayerContainer(
        ...
        playerOrder.indexOf(newOrder[playerIndex]),
        priorityOrder[j]
      );
...
function populateHandStatic(playerName, hand) {
  ...
  const playerHandContainer = document.querySelector(
    "#hand-container-" + playerName
  );
...
function updateHandNoAnimation(playerName, playerIndex, response) {
  ...
  // Create back-facing hand for others if not end of round / game
    ...
    for (let i = 0; i < response.hand_sizes[playerIndex]; i++) {
      ...
      document
        .querySelector("#hand-container-" + playerName)
...
function toggleHandSelectability(
  toggleOn,
  excludedClasses = [],
  excludedIds = []
) {
  const cardsInHand = document
    .querySelector("#hand-container-" + username)
...
function animateDraw(cardStr, player, handScore = 0) {
  ...
  document
    .querySelector("#hand-container-" + player)
  ...
  // Starting position
  const deckRect = document
    .querySelector("#deck-container")
  ...
  // Flip if for self
  if (player === username) {
    animObject = clone.animate(
      moveCard(deltaX, deltaY, "down", "up"),
      ANIMATION_TIMING
    );
  }
  // Keep face-down for other
    ...
    animObject = clone.animate(
      moveCard(deltaX, deltaY, "down", "down"),
      ANIMATION_TIMING
    );
  }
...
const RULES = {
  ...
  cribbage: "cribbage rules to come",
};
