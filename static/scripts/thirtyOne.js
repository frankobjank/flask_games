...
function animatePickup(cardStr, player, replaceDiscard, handScore) {
  ...
  // Flip down for other
    ...
    animObject = clone.animate(
      moveCard(deltaX, deltaY, "up", "down"),
      ANIMATION_TIMING
    );
  }
...
function updateDiscardNoAnimation(discardCard) {
  ...
  if (!discardCard || discardCard.length === 0) {
    ...
    document
      .querySelector("#discard-container")
      .replaceChildren(createPlaceholderCard("discard"));
  } else {
...
function updateThirtyOne(response) {
  ...
  if (inProgress) {
    ...
    if (
      playerOrder.length !==
      document.querySelectorAll(".player-container").length
    ) {
  ...
  for (actionObject of response.action_log) {
    ...
    console.log(
      `Action = ${actionObject.action}, player = ${actionObject.player}, card = ${actionObject.card}`
    );
  ...
  for (const player of playersConnected) {
    if (!response.player_order.includes(player)) {
      ...
      document.querySelector("#player-container-" + player).dataset.current =
        "0";
      // Reset `knocked`
      document.querySelector("#knocked-strong-" + player).innerText = "";
      document.querySelector("#player-container-" + player).dataset.knocked =
        "0";
    }
  }
  ...
  for (let i = 0; i < playerOrder.length; i++) {
    ...
    const playerContainer = document.querySelector(
      "#player-container-" + playerOrder[i]
    );
    ...
    document.querySelector("#lives-" + playerOrder[i]).innerText =
      "Extra Lives: ";
    ...
    if (response.lives[i] > 0) {
      ...
      document
        .querySelector("#lives-" + playerOrder[i])
        .appendChild(lifeStarsContainer);
    }
    ...
    if (username !== playerOrder[i]) {
      if (mode === "end_round" || mode === "end_game") {
        ...
        document.querySelector(
          "#hand-score-" + playerOrder[i]
        ).innerText = `Hand Score: ${response.final_scores[i]}`;
      } else {
      ...
      document.querySelector(
        "#hand-score-" + playerOrder[i]
      ).innerText = `Hand score: ${response.hand_score}`;
    }

    if (
      !document
        .querySelector("#hand-container-" + playerOrder[i])
        .hasChildNodes()
    ) {
